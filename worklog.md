...
Task: Build ONE React component file (`/home/z/my-project/src/components/designs/design-final.tsx`) — the client-chosen final design combining Design 3's warm, meet-the-owner vibe (Trusted Local Craftsman) with Design 2's photography-driven before/after showcase (Before/After Portfolio). Full production-ready handyman website with sticky nav, hero, trust bar, services, Meet Rick story, before/after case study spotlight, project gallery with lightbox, how-Rick-works, testimonials, trust badges band, lead form, FAQ, service area, final CTA, sticky footer. Warm orange palette (no indigo/blue/emerald). Uses shared photo + business modules + useLeadForm hook. No other files touched.

Work Log:
- Read worklog.md (Tasks 0/1/2-c/3) to understand prior agent work: 46 iCloud photos pulled + VLM-categorized, shared modules at `@/lib/handyman-photos` and `@/lib/business-info`, 3 design mockups already shipped on `/`, `useLeadForm` hook posts to /api/leads via Prisma.
- Read both reference design files end-to-end: `design-portfolio.tsx` (1168 lines, emerald accent, masonry gallery + dark case-study spotlight) and `design-trusted.tsx` (1274 lines, warm orange palette, meet-Rick narrative + Sheet mobile menu + contact form). Read `handyman-photos.ts` (PHOTOS, HERO_PICKS.power_washing = img_009.jpg, GALLERY_BY_CATEGORY, PHOTO_CAPTIONS, captionFor) and `business-info.ts` (BUSINESS brand/owner/phone/email/services/citiesServed/hours, TESTIMONIALS, FAQS).
- Read `use-lead-form.ts` hook: `useLeadForm({ design, successTitle, successDescription, onAfterSubmit })` returns `{ submit, submitting }`; falls back to a friendly toast if API unreachable so demo never breaks. Verified `design: 'trusted'` is a valid `DesignKey`.
- Wrote `design-final.tsx` with `'use client'` directive, exporting `DESIGN_ID = 'final'`, `DESIGN_NAME = 'R&R Handyman'`, default `DesignFinal`. Single-file component with small sub-components (TopNav, Hero, TrustBar, Services, MeetRick, CaseStudySpotlight, ProjectGallery, HowRickWorks, Testimonials, TrustBadgesBand, LeadForm, FAQ, ServiceArea, FinalCTA, Footer) for readability.
- Implemented all 15 sections per brief in this exact order: (1) sticky TopNav with `font-serif italic` "Rick Rodriguez" brand + Hammer badge + desktop center nav (Meet Rick / Services / Work / Reviews / Contact) + orange "Call Rick" button + Sheet mobile menu that closes on link click via `SheetClose asChild`; (2) Hero — two-column `md:grid-cols-2 md:min-h-[88vh]` with left column = badge "Springfield's neighborhood handyman", H1 "Hi, I'm Rick — your local handyman.", subhead w/ 12+ yrs + 1,850+ jobs + 4.9★, CTAs "Tell Rick What You Need" → #contact + "See My Work" → #work (with Images icon), trust row; right column = `HERO_PICKS.power_washing` (img_009.jpg) absolute-inset object-cover + orange "Before / After" Badge with Camera icon + overlapping owner Card on bottom-left with img_004.jpg avatar + "Rick Rodriguez · Owner · R&R Handyman Services" + "4.9★ · 12+ yrs"; (3) TrustBar on `bg-orange-50 border-y border-orange-200` — 4 inline stats (Licensed & Insured, 12+ Years, 1,850+ Jobs, 4.9★ from 213 Reviews); (4) Services — 3 cards (Plumbing/Carpentry/Power Washing) with image + orange "Starting at $X" badge + longBlurb + CheckCircle2 bullets in orange + "Ask Rick about {service}" link; (5) MeetRick story — 2-col with img_004.jpg (workers in front of garage shelving) + H2 + the 3 exact paragraphs from Design 3 ("I started R&R Handyman in 2012...", "Plumbing, carpentry, and power washing are the three things I do every week...", "Every job is backed by my 12-month workmanship guarantee...") + 3 trust pills; (6) CaseStudySpotlight — dark `bg-stone-900 text-stone-50` split: left img_026.jpg (deck board refinish before/after) + orange "Before / After" Badge, right "Project Spotlight: Deck Refinish" H3 + blurb about lifting weathered gray boards + customer quote from TESTIMONIALS[0] in a `border-l-2 border-orange-500` blockquote + "Request a deck quote" button; (7) ProjectGallery — H2 "Some of the jobs Rick's done lately." + 4 filter pills (All / Plumbing / Carpentry / Power Washing) wired to `useState<GalleryFilter>` + `useMemo(GALLERY_BY_CATEGORY[filter])` + masonry `columns-1 sm:columns-2 lg:columns-3 gap-4 [&>*]:mb-4` with break-inside-avoid cards (gradient overlay + category Badge + caption) + Dialog lightbox with `categoryLabelForSrc` Badge + DialogTitle + DialogDescription; (8) HowRickWorks — 3 steps with orange numbered circles (Phone / Calendar / Smile); (9) Testimonials — H2 "What the neighbors are saying." + TESTIMONIALS[0] + [2] cards with StarsRow + 5.0 + quote + initials avatar + name + location + service Badge; (10) TrustBadgesBand — same 4 stats as TrustBar on `bg-orange-50 border-y border-orange-200` (intentional bookend around testimonials); (11) LeadForm — `bg-stone-100`, 2-col: left contact rows (phone/email/hours/cities served with MapPin), right Card form (Name* / Phone* / Email optional / Select: Plumbing/Carpentry/Power Washing/Not sure / multiple / Textarea "Anything else Rick should know?" / orange full-width "Send Rick a Message" button with Loader2 spinner while submitting). Uses `useLeadForm({ design: 'trusted' })` which posts to /api/leads; onAfterSubmit bumps formKey to clear Radix Select; (12) FAQ — Accordion with FAQS on `bg-white`; (13) ServiceArea — H2 "Where Rick works." with map-style Card + city pills with MapPin icons + "Not on the list? Call..." on `bg-stone-100`; (14) FinalCTA — `bg-stone-900 text-stone-50`, H2 "Ready to get this off your to-do list?" + phone CTA + email CTA + "Text or call, 7 days a week"; (15) Footer — `bg-stone-900 text-stone-400 mt-auto`, 4-col grid (Brand + tagline + Licensed & Insured, Services list, Service area cities with MapPin, Hours + Contact with Clock/Phone/Mail icons), bottom row "© {CURRENT_YEAR} R&R Handyman Services. All rights reserved." + "Website by Setser Solutions" (developer credit per brief).
- Strict warm palette enforced throughout: `bg-stone-100` overall, `bg-white` cards, `bg-orange-50` accents, `bg-orange-600`/`text-orange-700`/`border-orange-200` accents, `bg-stone-900 text-stone-50` for dark sections. ZERO indigo/blue/emerald — verified via Grep scan of the file (`emerald|indigo|blue-[0-9]|sky-|cyan-|teal-|blue\b` → no matches). Amber-500 used only for the 5-star row (consistent with Design 3).
- Framer-motion used for subtle fade-up reveals (`fadeUp` + `stagger` + `Reveal` helper) on section headers, service cards, gallery cards, and the hero. All `whileInView` with `viewport={{ once: true }}` so reveals don't re-fire on scroll-back.
- Accessibility: skip-to-content link at top (orange-600 focus state), `<header><nav><main id="main"><section><article is implicit via motion.li/blockquote><footer>` semantics, every `<img>` has meaningful `alt` text (either `captionFor(src).title` for stock photos or explicit descriptions for hero / owner avatar), filter pills have `role="tab"` + `aria-selected`, mobile menu closes on link click via `SheetClose asChild`, all interactive elements have hover/focus states with `focus-visible:ring-orange-600`, Dialog provides `DialogTitle` + `DialogDescription` for screen readers, FAQ uses `Accordion` with proper `aria-*` semantics from Radix.
- Sticky footer pattern: outer `<div className="flex min-h-screen flex-col bg-stone-100">` wraps main + `<Footer className="mt-auto">` so the footer sticks to viewport bottom on short content and is pushed down naturally when content overflows.
- Lead form: `useLeadForm({ design: 'trusted' })` — reuses the existing /api/leads endpoint + Prisma persistence. On submit: validates name/phone/email client-side, posts JSON `{name,phone,email,service,message,design:'trusted',estimate}`, on success shows a `sonner` toast and resets the form (native reset + formKey bump to clear Radix Select). On API failure the hook still shows a friendly toast so the demo never looks broken.
- Gallery filter + lightbox verified: filter pills toggle `GalleryFilter` state, `useMemo` recomputes `sources` array, masonry grid re-renders with `key={filter}` so the stagger animation re-fires; clicking any card sets `lightboxSrc` and opens shadcn `Dialog` with full image + caption; Esc / backdrop / X close the dialog via `onOpenChange`. Lightbox uses `captionFor(src)` for title + blurb and `categoryLabelForSrc(src)` for the Badge.
- Mobile Sheet menu verified: SheetTrigger opens Sheet on right side, all 5 nav links are wrapped in `SheetClose asChild` so clicking any link both scrolls (via `href="#section"`) and closes the Sheet, plus a "Call {phone}" button at the bottom.
- Production-website only — NO mockup-review tooling (no switcher, no compare mode, no keyboard shortcuts). Component takes no props; it renders the full design top-to-bottom including its own footer.
- Ran `bun run lint` — zero errors / zero warnings. Dev server log shows clean `GET / 200` responses and `✓ Compiled in 236ms` after the new file was added. Did NOT touch `src/app/page.tsx`, `src/app/layout.tsx`, or any other file — only `design-final.tsx` was written. The main agent will integrate this as the active design on `/`.
- Also wrote work record to `/agent-ctx/14-a-final-combined-design.md` per project rules.
...
- Final design combines Design 3 (Trusted) narrative sections — TopNav, hero copy, Services, MeetRick, HowRickWorks, Testimonials, TrustBadgesBand, LeadForm, FAQ, ServiceArea, FinalCTA, Footer — with Design 2 (Portfolio) photography-driven sections — Hero right-column before/after image + owner card, dark CaseStudySpotlight, masonry ProjectGallery with Dialog lightbox.
- Strict warm palette: stone-100/white/orange-50 backgrounds, orange-600/orange-700/amber-500 accents, stone-900 dark sections. Verified zero indigo/blue/emerald via Grep.
